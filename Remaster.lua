...
-- Chat is connected after processCmd is defined.
-- =============================================================
-- Mobile autoresize 
-- =============================================================
...
function applyTheme(themeName)
	...
	if terminalDot then terminalDot.BackgroundColor3 = th.success end
	if terminalStatus then terminalStatus.TextColor3 = th.success end
	if terminalTitle then terminalTitle.TextColor3 = th.text end
	if cmdPrompt then cmdPrompt.TextColor3 = th.accent end
	if execBtn then execBtn.BackgroundColor3 = th.accent; execBtn.TextColor3 = th.tabTextActive end
	if cmdListBtn then cmdListBtn.BackgroundColor3 = th.btn; cmdListBtn.TextColor3 = th.text end
	if cmdClearBtn then cmdClearBtn.BackgroundColor3 = th.btn; cmdClearBtn.TextColor3 = th.text end
	if suggestionFrame then suggestionFrame.BackgroundColor3 = Color3.fromRGB(12, 12, 12) end
	if cmdListPanel then cmdListPanel.BackgroundColor3 = Color3.fromRGB(8, 8, 8) end
	if terminalOutput then terminalOutput.ScrollBarImageColor3 = th.accent end
	if suggestionScroll then suggestionScroll.ScrollBarImageColor3 = th.accent end
	if listScroll then listScroll.ScrollBarImageColor3 = th.accent end

	-- Core frames
	mainFrame.BackgroundColor3 = Color3.fromRGB(10,10,10)
	if topBar then topBar.BackgroundColor3 = Color3.fromRGB(32,32,32) end
	...
	if keyTab then keyTab.BackgroundColor3 = th.tabInactive; keyTab.TextColor3 = th.tabTextInactive end
	if uniTab then uniTab.BackgroundColor3 = th.tabInactive; uniTab.TextColor3 = th.tabTextInactive end

	-- Scroll frames & bars
	cmdBarFrame.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
	if cmdScroll then cmdScroll.BackgroundColor3 = Color3.fromRGB(5, 5, 5) end
	if setScroll then setScroll.BackgroundColor3 = Color3.fromRGB(0,0,0) end
	if keyScroll then keyScroll.BackgroundColor3 = Color3.fromRGB(0,0,0) end
	if uniScroll then uniScroll.BackgroundColor3 = Color3.fromRGB(0,0,0) end
	...
	if cmdFrame then cmdFrame.BackgroundColor3=Color3.fromRGB(0,0,0) end
	if terminalOutput then terminalOutput.BackgroundColor3=Color3.fromRGB(0,0,0) end
	if cmdBarFrame then cmdBarFrame.BackgroundColor3=Color3.fromRGB(16,16,16) end
	-- Sliders
	...
	-- Repair themed controls after the broad sweep so sliders, inputs and section text
	-- keep readable colors instead of inheriting unrelated theme roles.
	for _, root in ipairs({setScroll, uniScroll}) do
		if root then
			root.BackgroundColor3 = th.main
			for _, obj in ipairs(root:GetDescendants()) do
				if obj:IsA("TextButton") then
					obj.BackgroundColor3 = th.btn
					obj.TextColor3 = th.text
				elseif obj:IsA("TextBox") then
					obj.BackgroundColor3 = th.inputBg
					obj.TextColor3 = th.inputText
					obj.PlaceholderColor3 = Color3.fromRGB(130,130,140)
				elseif obj:IsA("TextLabel") then
					local parent = obj.Parent
					if parent and parent:IsA("Frame") and parent.Parent == root then
						obj.TextColor3 = th.sectionHeader
					else
						obj.TextColor3 = th.text
					end
				end
				if obj:GetAttribute("LunarBgRole") == "sliderTrack" then obj.BackgroundColor3 = th.sliderTrack end
				if obj:GetAttribute("LunarBgRole") == "sliderFill" then obj.BackgroundColor3 = th.sliderFill end
				if obj:GetAttribute("LunarBgRole") == "sliderKnob" then obj.BackgroundColor3 = th.sliderKnob end
				if obj:IsA("UIStroke") and obj:GetAttribute("LunarStrokeRole") == "accent" then obj.Color = th.accent end
			end
		end
	end
	...
	if applyModernControlStyles then pcall(applyModernControlStyles) end
...
local notifWidth = 390
local notifHeight = 86
local notifSpacing = 8
local startY = 16
local notifOffscreen = 410
...
if isMobile then
	...
	local side = math.min(notifViewport.X, notifViewport.Y)
	if side < 700 then
		notifWidth = 290
		notifHeight = 70
		notifSpacing = 6
...
local function notify(text, col)
	...
	local function dismiss()
		if closed or not f.Parent then return end
		closed=true
		for i,n in ipairs(activeNotifications) do if n==f then table.remove(activeNotifications,i);break end end
		TweenService:Create(f,TweenInfo.new(0.22,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{Position=UDim2.new(1,notifOffscreen,0,f.Position.Y.Offset),BackgroundTransparency=1}):Play()
		TweenService:Create(message,TweenInfo.new(0.16),{TextTransparency=1}):Play()
		task.delay(0.24,function() if f.Parent then f:Destroy() end;repositionAll() end)
	end
...
-- Check mobile
local flingViewport = workspace.CurrentCamera.ViewportSize
if UserInputService.TouchEnabled and (not UserInputService.KeyboardEnabled or not UserInputService.MouseEnabled or flingViewport.X < 700 or flingViewport.Y < 500) then
...
if isMobile then
	if smallestSide<600 then scale=0.48;fontScale=0.78 elseif smallestSide<800 then scale=0.58;fontScale=0.84 else scale=0.68;fontScale=0.9 end
else scale=1;fontScale=1 end
...
topBar=Instance.new("Frame",mainFrame);topBar.Size=UDim2.new(1,0,0,math.floor(38*scale));topBar.BackgroundColor3=Color3.fromRGB(32,32,32);topBar.BorderSizePixel=0;topBar.ZIndex=2147483647
...
UserInputService.InputChanged:Connect(function(input)
	if draggingWindow and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) and dragStart and windowStart then
		local delta=input.Position-dragStart
		local target=UDim2.new(windowStart.X.Scale,windowStart.X.Offset+delta.X,windowStart.Y.Scale,windowStart.Y.Offset+delta.Y)
		mainFrame.Position=mainFrame.Position:Lerp(target,0.28)
	end
end)
UserInputService.InputEnded:Connect(function(input)
...
minBtn.MouseButton1Click:Connect(function() minimized=not minimized;if minimized then mainFrame:TweenSize(UDim2.new(0,math.floor(640*scale),0,math.floor(38*scale)),Enum.EasingDirection.Out,Enum.EasingStyle.Quint,0.24,true);minBtn.Text="+" else mainFrame:TweenSize(origSize,Enum.EasingDirection.Out,Enum.EasingStyle.Quint,0.24,true);minBtn.Text="−" end end)

resizeHandle=Instance.new("TextButton",mainFrame);resizeHandle.Name="ResizeHandle";resizeHandle.Size=UDim2.fromOffset(math.floor(18*scale),math.floor(18*scale));resizeHandle.Position=UDim2.new(1,math.floor(-18*scale),1,math.floor(-18*scale));resizeHandle.BackgroundColor3=Color3.fromRGB(48,48,48);resizeHandle.Text="◢";resizeHandle.Font=Enum.Font.Code;resizeHandle.TextSize=math.floor(9*fontScale);resizeHandle.TextColor3=Color3.fromRGB(130,130,130);resizeHandle.BorderSizePixel=0;resizeHandle.AutoButtonColor=false;resizeHandle.ZIndex=2147483647
resizing=false;startSize=nil;startPos=nil
resizeHandle.InputBegan:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then resizing=true;startSize=mainFrame.Size;startPos=input.Position end end)

tabBar=Instance.new("Frame",mainFrame);tabBar.Size=UDim2.new(1,math.floor(-20*scale),0,math.floor(36*scale));tabBar.Position=UDim2.fromOffset(math.floor(10*scale),math.floor(48*scale));tabBar.BackgroundColor3=Color3.fromRGB(22,22,22);tabBar.BorderSizePixel=0;tabBar.ZIndex=2147483647
tabLayout=Instance.new("UIListLayout",tabBar);tabLayout.FillDirection=Enum.FillDirection.Horizontal;tabLayout.Padding=UDim.new(0,1);tabLayout.SortOrder=Enum.SortOrder.LayoutOrder
function makeTab(name,text) local b=Instance.new("TextButton",tabBar);b.Name=name;b.Size=UDim2.new(0,math.floor(112*scale),1,0);b.BackgroundColor3=Color3.fromRGB(22,22,22);b.Text=text;b.Font=Enum.Font.Code;b.TextSize=math.floor(11*fontScale);b.TextColor3=Color3.fromRGB(185,185,185);b.BorderSizePixel=0;b.AutoButtonColor=false;b.ZIndex=2147483647;return b end
cmdTab=makeTab("CmdTab","Commands");setTab=makeTab("SetTab","Settings");keyTab=makeTab("KeyTab","Keybinds");uniTab=makeTab("UniTab","Universal")

contentFrame=Instance.new("Frame",mainFrame);contentFrame.Name="Content";contentFrame.Size=UDim2.new(1,math.floor(-20*scale),1,math.floor(-94*scale));contentFrame.Position=UDim2.fromOffset(math.floor(10*scale),math.floor(94*scale));contentFrame.BackgroundTransparency=1;contentFrame.ClipsDescendants=true;contentFrame.ZIndex=2147483647

cmdFrame=Instance.new("Frame",contentFrame);cmdFrame.Name="CmdFrame";cmdFrame.Size=UDim2.fromScale(1,1);cmdFrame.BackgroundColor3=Color3.fromRGB(0,0,0);cmdFrame.BorderSizePixel=0;cmdFrame.ZIndex=2147483647
terminalRoot=cmdFrame
terminalOutput=Instance.new("ScrollingFrame",terminalRoot);terminalOutput.Name="Output";terminalOutput.Size=UDim2.new(1,math.floor(-2*scale),1,math.floor(-58*scale));terminalOutput.Position=UDim2.fromOffset(1,1);terminalOutput.BackgroundColor3=Color3.fromRGB(0,0,0);terminalOutput.BorderSizePixel=0;terminalOutput.ScrollBarThickness=math.max(2,math.floor(4*scale));terminalOutput.ScrollBarImageColor3=currentTheme.accent;terminalOutput.AutomaticCanvasSize=Enum.AutomaticSize.Y;terminalOutput.ZIndex=2147483647;cmdScroll=terminalOutput
cmdList=Instance.new("UIListLayout",terminalOutput);cmdList.Padding=UDim.new(0,1);cmdList.SortOrder=Enum.SortOrder.LayoutOrder
terminalPad=Instance.new("UIPadding",terminalOutput);terminalPad.PaddingTop=UDim.new(0,math.floor(7*scale));terminalPad.PaddingBottom=UDim.new(0,math.floor(7*scale));terminalPad.PaddingLeft=UDim.new(0,math.floor(9*scale));terminalPad.PaddingRight=UDim.new(0,math.floor(9*scale))
terminalHistory={};terminalHistoryIndex=1;terminalCommandCount=0
function LunarTerminal_addLine(text,role,muted,typeSpeed)
	if not terminalOutput or not terminalOutput.Parent then return end
	local line=Instance.new("TextLabel");line.Name="TerminalLine";line.Size=UDim2.new(1,math.floor(-8*scale),0,math.floor(18*scale));line.AutomaticSize=Enum.AutomaticSize.Y;line.BackgroundTransparency=1;line.Text="";line.Font=Enum.Font.Code;line.TextSize=math.floor(12*fontScale);line.TextColor3=role or Color3.fromRGB(220,220,220);line.TextTransparency=muted and 0.15 or 0;line.TextXAlignment=Enum.TextXAlignment.Left;line.TextYAlignment=Enum.TextYAlignment.Center;line.TextWrapped=true;line.ZIndex=2147483647;line.LayoutOrder=#terminalOutput:GetChildren();line.Parent=terminalOutput
	local fullText=tostring(text or "")
	if typeSpeed and typeSpeed>0 and #fullText>0 then task.spawn(function() for i=1,#fullText do if not line.Parent then return end;line.Text=fullText:sub(1,i);task.wait(typeSpeed) end end) else line.Text=fullText end
...
suggestionFrame=Instance.new("Frame",terminalRoot);suggestionFrame.Name="Suggestions";suggestionFrame.Size=UDim2.new(1,math.floor(-170*scale),0,math.floor(170*scale));suggestionFrame.Position=UDim2.new(0,math.floor(76*scale),1,math.floor(-230*scale));suggestionFrame.BackgroundColor3=Color3.fromRGB(10,10,10);suggestionFrame.BorderSizePixel=0;suggestionFrame.Visible=false;suggestionFrame.ZIndex=2147483647
suggestionScroll=Instance.new("ScrollingFrame",suggestionFrame);suggestionScroll.Size=UDim2.fromScale(1,1);suggestionScroll.BackgroundTransparency=1;suggestionScroll.BorderSizePixel=0;suggestionScroll.ScrollBarThickness=math.max(2,math.floor(3*scale));suggestionScroll.ScrollBarImageColor3=currentTheme.accent;suggestionScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y;suggestionScroll.ZIndex=2147483647
suggestionLayout=Instance.new("UIListLayout",suggestionScroll);suggestionLayout.Padding=UDim.new(0,1)
cmdListPanel=Instance.new("Frame",terminalRoot);cmdListPanel.Name="CommandList";cmdListPanel.Size=UDim2.new(0,math.floor(290*scale),1,0);cmdListPanel.Position=UDim2.new(1,0,0,0);cmdListPanel.BackgroundColor3=Color3.fromRGB(8,8,8);cmdListPanel.BorderSizePixel=0;cmdListPanel.Visible=false;cmdListPanel.ZIndex=2147483647
cmdPanelHeader=Instance.new("TextLabel",cmdListPanel);cmdPanelHeader.Size=UDim2.new(1,0,0,math.floor(36*scale));cmdPanelHeader.BackgroundColor3=Color3.fromRGB(30,30,30);cmdPanelHeader.Text="COMMAND INDEX";cmdPanelHeader.Font=Enum.Font.Code;cmdPanelHeader.TextSize=math.floor(11*fontScale);cmdPanelHeader.TextColor3=Color3.fromRGB(235,235,235);cmdPanelHeader.TextXAlignment=Enum.TextXAlignment.Left;cmdPanelHeader.ZIndex=2147483647
panelClose=Instance.new("TextButton",cmdListPanel);panelClose.Size=UDim2.fromOffset(math.floor(30*scale),math.floor(36*scale));panelClose.Position=UDim2.new(1,math.floor(-30*scale),0,0);panelClose.BackgroundTransparency=1;panelClose.Text="<";panelClose.Font=Enum.Font.Code;panelClose.TextSize=math.floor(14*fontScale);panelClose.TextColor3=Color3.fromRGB(200,200,200);panelClose.ZIndex=2147483647
listScroll=Instance.new("ScrollingFrame",cmdListPanel);listScroll.Size=UDim2.new(1,0,1,math.floor(-36*scale));listScroll.Position=UDim2.fromOffset(0,math.floor(36*scale));listScroll.BackgroundTransparency=1;listScroll.BorderSizePixel=0;listScroll.ScrollBarThickness=math.max(2,math.floor(4*scale));listScroll.ScrollBarImageColor3=currentTheme.accent;listScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y;listScroll.ZIndex=2147483647
listLayout=Instance.new("UIListLayout",listScroll);listLayout.Padding=UDim.new(0,1)
allCommands={"!aimbot","!bang","!unbang","!boombox","!camlock","!uncamlock","!console","!cmdbar","!copychat","!uncopychat","!crosshair","!uncrosshair","!unload","!disablefalldamage","!enable inventory","!enable playerlist","!esp all","!unesp all","!explode","!fire","!unfire","!firstp","!fling","!unfling","!flashlight","!unflashlight","!fly","!unfly","!freecam","!unfreecam","!freeze","!unfreeze","!gravity","!resetgravity","!infjump","!uninfjump","!joinlogs","!jerk","!unjerk","!jump","!kill","!lay","!leave","!logs","!loopgoto","!unloopgoto","!tpwalk","!untpwalk","!noclip","!unnoclip","!mm2","!orbit","!unorbit","!ragdoll","!unragdoll","!rejoin","!removewaypoint","!resettime","!sit","!speed","!serverhop","!spin","!unspin","!stopwatch","!sunglare","!unsunglare","!superjump","!unsuperjump","!thirdp","!timeset","!to","!trip","!tracers","!untracers","!vehiclefly","!unvehiclefly","!view","!unview","!volume","!waypoint","!walkonwater","!unwalkonwater","!xray","!unxray","!zoom","!unzoom","!fov"}
cmdDesc={ ["!aimbot"]="Open the aimbot panel",["!bang"]="Run the configured target action",["!boombox"]="Open the boombox system",["!camlock"]="Lock camera to a player",["!console"]="Open developer console",["!crosshair"]="Enable Lunar crosshair",["!explode"]="Run the target explode command",["!fire"]="Add the target fire effect",["!firstp"]="Enable first person",["!fling"]="Open touch fling controls",["!flashlight"]="Toggle flashlight",["!fly"]="Enable flying",["!freecam"]="Enable free camera",["!freeze"]="Freeze a target",["!gravity"]="Set workspace gravity",["!infjump"]="Enable infinite jumping",["!joinlogs"]="Open join logs",["!jump"]="Jump your character",["!kill"]="Run the target kill command",["!lay"]="Lay down",["!leave"]="Leave the game",["!logs"]="Open logs",["!noclip"]="Enable noclip",["!mm2"]="Open MM2 tools",["!rejoin"]="Rejoin the current server",["!resetgravity"]="Reset workspace gravity",["!sit"]="Sit your character",["!serverhop"]="Join another server",["!spin"]="Spin your character",["!stopwatch"]="Open stopwatch",["!superjump"]="Enable super jump",["!thirdp"]="Enable third person",["!to"]="Teleport to a player",["!tracers"]="Enable tracers",["!vehiclefly"]="Enable vehicle flight",["!view"]="Spectate a player",["!volume"]="Set game volume",["!waypoint"]="Create a waypoint",["!walkonwater"]="Walk across water",["!xray"]="Enable x-ray visibility"}
commandDescriptions=cmdDesc;commandTargetCommands={bang=true,camlock=true,copychat=true,esp=true,unesp=true,explode=true,fire=true,unfire=true,freeze=true,kill=true,noclip=true,unnoclip=true,orbit=true,loopgoto=true,speed=true,to=true,trip=true,view=true,unfreeze=true,unfly=true,unragdoll=true,unfling=true}
function fuzzyScore(query,candidate) local q=string.lower(query or "");local c=string.lower(candidate or "");if q=="" then return 0 end;if q==c then return 1000 end;if c:sub(1,#q)==q then return 800-(#c-#q) end;local at=c:find(q,1,true);if at then return 600-at end;local i,j=1,1;while i<=#q and j<=#c do if q:sub(i,i)==c:sub(j,j) then i+=1 end;j+=1 end;if i>#q then return 350-(#c-#q) end;return -1 end
...
panelClose.MouseButton1Click:Connect(function() TweenService:Create(cmdListPanel,TweenInfo.new(0.2,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{Position=UDim2.new(1,0,0,0)}):Play();task.delay(0.21,function() if cmdListPanel then cmdListPanel.Visible=false end end) end)
function buildCommandList() for _,child in ipairs(listScroll:GetChildren()) do if child:IsA("TextButton") then child:Destroy() end end;for _,command in ipairs(allCommands) do row=Instance.new("TextButton");row.Size=UDim2.new(1,math.floor(-6*scale),0,math.floor(42*scale));row.BackgroundColor3=Color3.fromRGB(14,14,14);row.Text="  "..command.."\n  "..(cmdDesc[command] or "");row.Font=Enum.Font.Code;row.TextSize=math.floor(9*fontScale);row.TextColor3=Color3.fromRGB(205,205,205);row.TextXAlignment=Enum.TextXAlignment.Left;row.BorderSizePixel=0;row.AutoButtonColor=false;row.ZIndex=2147483647;row.Parent=listScroll;row.MouseEnter:Connect(function() row.BackgroundColor3=Color3.fromRGB(30,30,30) end);row.MouseLeave:Connect(function() row.BackgroundColor3=Color3.fromRGB(14,14,14) end);row.MouseButton1Click:Connect(function() cmdInput.Text=command;cmdListPanel.Visible=false;cmdInput:CaptureFocus() end) end end
buildCommandList()
-- ========== SETTINGS TAB ==========
...
setList.Padding = UDim.new(0, math.floor(8 * scale))
setList.SortOrder = Enum.SortOrder.LayoutOrder
setPad = Instance.new("UIPadding", setScroll)
setPad.PaddingTop = UDim.new(0, math.floor(6 * scale))
setPad.PaddingBottom = UDim.new(0, math.floor(6 * scale))
setPad.PaddingLeft = UDim.new(0, math.floor(5 * scale))
setPad.PaddingRight = UDim.new(0, math.floor(5 * scale))

-- Section creator
function makeSection(parent, titleText, h)
	local s = Instance.new("Frame", parent)
	s.Size = UDim2.new(1, math.floor(-10 * scale), 0, math.floor(h * scale))
	s.BackgroundColor3 = Color3.fromRGB(12, 12, 12)
	s.BorderSizePixel = 0
...
blurLabel.TextSize = math.floor(12*fontScale)
blurLabel.TextColor3 = globalConfig.textColor
...
cornerLabel.TextSize=math.floor(12*fontScale)
...
local sectionHeight = math.floor(36 * scale) + math.floor(rows * 88 * scale) + math.floor(10 * scale)
thSection.Size = UDim2.new(1, math.floor(-16 * scale), 0, sectionHeight)

thGrid = Instance.new("UIGridLayout", thCont)
thGrid.CellSize = UDim2.new(0.46, 0, 0, math.floor(74 * scale))
thGrid.CellPadding = UDim2.new(0, math.floor(14 * scale), 0, math.floor(14 * scale))
...
-- Keybinds tab
keyFrame=Instance.new("Frame",contentFrame)
keyFrame.Name="KeybindsFrame"
keyFrame.Size=UDim2.fromScale(1,1)
keyFrame.BackgroundColor3=Color3.fromRGB(0,0,0)
keyFrame.BorderSizePixel=0
keyFrame.Visible=false
keyFrame.ZIndex=2147483647
keyScroll=Instance.new("ScrollingFrame",keyFrame)
keyScroll.Size=UDim2.fromScale(1,1)
keyScroll.BackgroundColor3=Color3.fromRGB(0,0,0)
keyScroll.BorderSizePixel=0
keyScroll.ScrollBarThickness=math.max(2,math.floor(4*scale))
keyScroll.ScrollBarImageColor3=currentTheme.accent
keyScroll.AutomaticCanvasSize=Enum.AutomaticSize.Y
keyScroll.ZIndex=2147483647
keyList=Instance.new("UIListLayout",keyScroll)
keyList.Padding=UDim.new(0,math.floor(5*scale))
keyList.SortOrder=Enum.SortOrder.LayoutOrder
keyPad=Instance.new("UIPadding",keyScroll)
keyPad.PaddingTop=UDim.new(0,math.floor(8*scale));keyPad.PaddingBottom=UDim.new(0,math.floor(8*scale));keyPad.PaddingLeft=UDim.new(0,math.floor(5*scale));keyPad.PaddingRight=UDim.new(0,math.floor(5*scale))
keyInfo=Instance.new("TextLabel",keyScroll)
keyInfo.Size=UDim2.new(1,math.floor(-10*scale),0,math.floor(52*scale))
keyInfo.BackgroundColor3=Color3.fromRGB(12,12,12)
keyInfo.Text="KEYBINDS\nClick None to enter capture mode, then press a keyboard key. Click the assigned key again to execute the command."
keyInfo.Font=Enum.Font.Code
keyInfo.TextSize=math.floor(10*fontScale)
keyInfo.TextColor3=Color3.fromRGB(185,185,185)
...
keybindRows={}
function makeKeybindRow(displayName,command,order)
	local row=Instance.new("Frame",keyScroll)
	row.Name="Keybind_"..displayName:gsub("%s+","_");row.Size=UDim2.new(1,math.floor(-10*scale),0,math.floor(48*scale));row.BackgroundColor3=Color3.fromRGB(12,12,12);row.BorderSizePixel=0;row.LayoutOrder=order;row.ZIndex=2147483647
	local stroke=Instance.new("UIStroke",row);stroke.Color=Color3.fromRGB(48,48,48);stroke.Thickness=1
	local name=Instance.new("TextLabel",row);name.Size=UDim2.new(0.42,0,1,0);name.Position=UDim2.fromOffset(math.floor(12*scale),0);name.BackgroundTransparency=1;name.Text=displayName;name.Font=Enum.Font.Code;name.TextSize=math.floor(11*fontScale);name.TextColor3=Color3.fromRGB(225,225,225);name.TextXAlignment=Enum.TextXAlignment.Left;name.ZIndex=2147483647
	local cmd=Instance.new("TextLabel",row);cmd.Size=UDim2.new(0.27,0,1,0);cmd.Position=UDim2.new(0.43,0,0,0);cmd.BackgroundTransparency=1;cmd.Text=command;cmd.Font=Enum.Font.Code;cmd.TextSize=math.floor(9*fontScale);cmd.TextColor3=Color3.fromRGB(105,105,105);cmd.TextXAlignment=Enum.TextXAlignment.Left;cmd.ZIndex=2147483647
	local btn=Instance.new("TextButton",row);btn.Name="KeyButton";btn.Size=UDim2.new(0,math.floor(95*scale),0,math.floor(32*scale));btn.Position=UDim2.new(1,math.floor(-105*scale),0.5,math.floor(-16*scale));btn.BackgroundColor3=Color3.fromRGB(28,28,28);btn.Text=keybindData[command] and tostring(keybindData[command]) or "None";btn.Font=Enum.Font.Code;btn.TextSize=math.floor(10*fontScale);btn.TextColor3=currentTheme.accent;btn.BorderSizePixel=0;btn.AutoButtonColor=false;btn.ZIndex=2147483647
	local bs=Instance.new("UIStroke",btn);bs.Color=Color3.fromRGB(65,65,65);bs.Thickness=1
	btn.MouseEnter:Connect(function() btn.BackgroundColor3=Color3.fromRGB(42,42,42) end);btn.MouseLeave:Connect(function() btn.BackgroundColor3=Color3.fromRGB(28,28,28) end)
	btn.MouseButton1Click:Connect(function()
		if keybindCapture==command then
			keybindCapture=nil
			btn.Text=keybindData[command] and tostring(keybindData[command]) or "None"
			return
		end
		if keybindData[command] then
			executeKeybind(command)
			return
		end
		keybindCapture=command
		btn.Text="PRESS KEY"
	end)
	table.insert(keybindRows,{command=command,button=btn})
end
...
uniList.Padding = UDim.new(0, math.floor(7 * scale))
uniList.SortOrder = Enum.SortOrder.LayoutOrder
uniPad = Instance.new("UIPadding", uniScroll)
uniPad.PaddingTop = UDim.new(0, math.floor(6 * scale))
uniPad.PaddingBottom = UDim.new(0, math.floor(6 * scale))
uniPad.PaddingLeft = UDim.new(0, math.floor(5 * scale))
uniPad.PaddingRight = UDim.new(0, math.floor(5 * scale))
...
function qBtn(parent, name, label, colorKey, callback)
...
UserInputService.InputChanged:Connect(function(input)
	if resizing and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) and startSize and startPos then d=input.Position-startPos;mainFrame.Size=UDim2.new(0,math.clamp(startSize.X.Offset+d.X,math.floor(380*scale),math.floor(900*scale)),0,math.clamp(startSize.Y.Offset+d.Y,math.floor(360*scale),math.floor(820*scale))) end
	if blurDragging and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then setBlurFromX(input.Position.X) end
	if cornerDragging and (input.UserInputType==Enum.UserInputType.MouseMovement or input.UserInputType==Enum.UserInputType.Touch) then setCornerFromX(input.Position.X) end
end)
UserInputService.InputEnded:Connect(function(input) if input.UserInputType==Enum.UserInputType.MouseButton1 or input.UserInputType==Enum.UserInputType.Touch then resizing=false;blurDragging=false;cornerDragging=false end end)
function switchTab(which) cmdFrame.Visible=which=="Commands";setFrame.Visible=which=="Settings";keyFrame.Visible=which=="Keybinds";uniFrame.Visible=which=="Universal";for _,tab in ipairs({cmdTab,setTab,keyTab,uniTab}) do tab.BackgroundColor3=Color3.fromRGB(22,22,22);tab.TextColor3=Color3.fromRGB(185,185,185) end;active=which=="Commands" and cmdTab or which=="Settings" and setTab or which=="Keybinds" and keyTab or uniTab;active.BackgroundColor3=Color3.fromRGB(45,45,45);active.TextColor3=currentTheme.accent end
cmdTab.MouseButton1Click:Connect(function() switchTab("Commands") end);setTab.MouseButton1Click:Connect(function() switchTab("Settings") end);keyTab.MouseButton1Click:Connect(function() switchTab("Keybinds") end);uniTab.MouseButton1Click:Connect(function() switchTab("Universal") end)
switchTab("Commands")
UserInputService.InputBegan:Connect(function(input,gameProcessed)
	...
	if gameProcessed then return end
	if input.KeyCode==Enum.KeyCode.F2 then if lunarGui then lunarGui.Enabled=true;cmdInput:CaptureFocus() end;return end
	if input.KeyCode==Enum.KeyCode.F3 then if lunarGui and lunarGui.Enabled then cmdListPanel.Visible=true;cmdListPanel.Position=UDim2.new(1,0,0,0);TweenService:Create(cmdListPanel,TweenInfo.new(0.2,Enum.EasingStyle.Quint,Enum.EasingDirection.Out),{Position=UDim2.new(1,math.floor(-290*scale),0,0)}):Play() end;return end
	if input.KeyCode==Enum.KeyCode.F4 then LunarTerminal_clearOutput();return end
	if input.KeyCode==Enum.KeyCode.RightShift then if lunarGui then lunarGui.Enabled=not lunarGui.Enabled;if lunarGui.Enabled then playOpen() else playClose() end end;return end
...
function applyModernControlStyles()
	...
	local function corner(obj, radius)
		...
		c.CornerRadius=UDim.new(0,math.floor(radius*scale))
	end
	local function stroke(obj, color, transparency, thickness)
		local st=obj:FindFirstChildOfClass("UIStroke")
		if not st then st=Instance.new("UIStroke",obj) end
		st.Color=color
		st.Transparency=transparency
		st.Thickness=thickness
	end
	local function button(btn, base, textColor)
		btn.AutoButtonColor=false
		btn.BackgroundColor3=base or currentTheme.btn
		btn.TextColor3=textColor or currentTheme.text
		btn.BorderSizePixel=0
		btn.ZIndex=2147483647
		corner(btn,7)
	...
	if setScroll then
		for _,obj in ipairs(setScroll:GetDescendants()) do
			if obj:IsA("TextButton") then
				button(obj,currentTheme.btn,obj.TextColor3)
			elseif obj:IsA("TextBox") then
				obj.BackgroundColor3=Color3.fromRGB(24,24,30)
				obj.BorderSizePixel=0
				corner(obj,7)
				stroke(obj,Color3.fromRGB(65,65,74),0.4,1)
			end
		end
		if setScroll:FindFirstChildOfClass("UIListLayout") then
			setScroll:FindFirstChildOfClass("UIListLayout").Padding=UDim.new(0,math.floor(10*scale))
		end
	end
	if uniScroll then
		for _,obj in ipairs(uniScroll:GetDescendants()) do
			if obj:IsA("TextButton") then
				local base=currentTheme.btn
				if obj.Name=="NoclipBtn" then base=currentTheme.toggleOff end
				button(obj,base,currentTheme.text)
			elseif obj:IsA("TextBox") then
				obj.BackgroundColor3=Color3.fromRGB(24,24,30)
				obj.BorderSizePixel=0
				corner(obj,7)
	...
	if thCont then
		for _,card in ipairs(thCont:GetChildren()) do
			if card:IsA("TextButton") then
				local th=themes[card.Name:gsub("ThemeCard$","")]
				if th then
					card.BackgroundColor3=Color3.fromRGB(17,17,21)
					card.BorderSizePixel=0
					corner(card,10)
					local st=card:FindFirstChildOfClass("UIStroke")
					if st then st.Color=th.accent;st.Transparency=0.58;st.Thickness=1.2 end
					local preview=card:FindFirstChild("Preview")
					if preview then
						preview.Size=UDim2.new(1,0,0,math.floor(24*scale))
						preview.BackgroundColor3=th.accent
						corner(preview,10)
					end
					local nameLabel=card:FindFirstChild("Name")
					if nameLabel then
						nameLabel.Position=UDim2.fromOffset(math.floor(12*scale),math.floor(31*scale))
						nameLabel.Size=UDim2.new(1,math.floor(-36*scale),0,math.floor(20*scale))
						nameLabel.TextXAlignment=Enum.TextXAlignment.Left
						nameLabel.TextColor3=Color3.fromRGB(235,235,240)
					end
					local dot=card:FindFirstChild("ActiveDot")
					if dot then
						dot.Size=UDim2.fromOffset(math.floor(8*scale),math.floor(8*scale))
						dot.Position=UDim2.new(1,math.floor(-16*scale),0,math.floor(33*scale))
						dot.BackgroundColor3=th.accent
					end
					local oldSwatches=card:FindFirstChild("ThemeSwatches")
					if not oldSwatches then
						oldSwatches=Instance.new("Frame",card)
						oldSwatches.Name="ThemeSwatches"
						oldSwatches.BackgroundTransparency=1
						oldSwatches.Size=UDim2.new(1,math.floor(-24*scale),0,math.floor(10*scale))
						oldSwatches.Position=UDim2.new(0,math.floor(12*scale),1,math.floor(-16*scale))
						oldSwatches.ZIndex=2147483647
						local colors={th.accent,th.btn,th.list}
						for i,col in ipairs(colors) do
							local sw=Instance.new("Frame",oldSwatches)
							sw.Size=UDim2.fromOffset(math.floor(18*scale),math.floor(8*scale))
							sw.Position=UDim2.fromOffset(math.floor((i-1)*22*scale),0)
							sw.BackgroundColor3=col
							sw.BorderSizePixel=0
						sw.ZIndex=2147483647
						corner(sw,4)
						stroke(sw,Color3.fromRGB(255,255,255),0.72,1)
						end
					end
					card.MouseEnter:Connect(function()
						local st=card:FindFirstChildOfClass("UIStroke")
						if st then TweenService:Create(st,TweenInfo.new(0.14),{Transparency=0.05,Thickness=2}):Play() end
						TweenService:Create(card,TweenInfo.new(0.14),{BackgroundColor3=Color3.fromRGB(23,23,28)}):Play()
					end)
					card.MouseLeave:Connect(function()
						local st=card:FindFirstChildOfClass("UIStroke")
						if st then TweenService:Create(st,TweenInfo.new(0.16),{Transparency=0.58,Thickness=1.2}):Play() end
						TweenService:Create(card,TweenInfo.new(0.16),{BackgroundColor3=Color3.fromRGB(17,17,21)}):Play()
					end)
				end
			end
		end
